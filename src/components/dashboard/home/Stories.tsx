      ...
      <div
        ...
        {Array.from(stories).map((_: any, index) => (
          <div key={index} style={{ flex: "0 0 auto" }} className="mt-10 mx-5">
            ...
            <div
              className="w-[296px] mt-3 flex flex-col justify-end items-center h-[526px] rounded-md relative"
              style={{
                background: `url(${_.player[0]?.content?.source}), lightgray 0px 0px / 100% 100% no-repeat`,
                backgroundSize: "cover",
              }}>
              <div
                className="w-[250px] p-4 mb-2  rounded-[15px] bg-[#fff]"
                style={{
                  boxShadow: "4px 4px 20px 0px rgba(36, 36, 80, 0.10)",
                }}>
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 ml-5">
                    ...
                    <button
                      className="px-1 mt-2 py-2 text-white rounded-md"
                      style={{
                        backgroundColor:
                          _?.player[0]?.layout?.cta?.backgroundColor,
                      }}>
                      {(_?.player[0]?.layout?.cta?.text).toUpperCase()}
                    </button>
                  </div>
                </div>
              </div>
            </div>
            <p className="text-center mt-1 text-lg w-[296px]">
